...
const PropertyForm = ({ onDataChange }) => {
  ...
  useEffect(() => {
    if (onDataChange) {
      onDataChange(propertyData, imagesProperty);
    }
  }, [propertyData, imagesProperty, onDataChange]);
  ...
  const validationSchema = Yup.object().shape({
    ...
    address: Yup.string().required(
      "SVP indiquer une adresse commune de sélection"
    ),
    ...
    switch (propertyData.offer) {
      case "1":
        if (propertyData.furnished === "1") {
          ...
          ["8", "9", "10", "11", "12", "19", "20"].includes(propertyData.type)
        ) {
          usage = 3;
        } else {
          usage = 1;
    ...
    switch (propertyData.offer) {
      case "1":
        if (propertyData.furnished === "1") {
          ...
          ["8", "9", "11", "12", "19", "29", "30"].includes(propertyData.type)
        ) {
  ...
  // MOVE buildAppendMapFileUpload function to MultiStepPropertyForm.js
  // And make neccessary chnages
  ...
  //     JSON.stringify({
  //       query:
  //         "mutation AddPropertyImage($data: ProprieteInput!) { enrollProperty(input: $data) }",
  ...
  //         setPropertyCreatedNotification(
  //           "Le bien immobilier a été listé avec succès. Consultez votre portofolio immobilier pour détails."
  //         );
  //         const propertyId = response.data.data.enrollProperty.id;
  //         router.push(`/tg/account-agents?propertyId=${propertyId}`);
  //       } else {
  //         setPropertyCreatedNotification(
  //           "Réponse inattendue du serveur. Redirection..."
  //         );
  //         router.push(`/tg/account-agents?propertyId=unknown`);
  //       }
  //     })
  //     .catch((error) => {
  ...
  const onSubmit = (data, event) => {
    ...
    // It should no longer call buildAppendMapFileUpload directly.
    // Data validation happens via yupSchema.
    // The actual data for the step is collected via onDataChange.
    // The "Save and Continue" button in PropertyFormStep will trigger the actual step progression.
    console.log("PropertyForm RHF onSubmit triggered, data:", data);
    // We can perform any final internal validation or data processing if needed
    // before onDataChange (which is already called via useEffect) sends it up.
  ...
  const handleFilePondUpdate = (fileItems) => {
    setImagesProperty(fileItems.map((fileItem) => fileItem.file));
  };
  ...
  const propertyManagerOptions = [
    {
      ...
      label: "Je suis le gestionnaire ou proprietaire",
    },
  ];

  const { status: villesStatus, data: villes } = useListTowns(228);
  const { status: quartiersStatus, data: quartiers } = useListQuarters(
    Number(propertyData?.town)
  );
  ...
  const {
    ...
    data: landlords1232,
    error: error1232,
    isFetching: isFetching1232,
    isLoading: isLoading1232,
    isError: isError1232,
  } = useLandLord(1232);
  ...
  const propertyOwnerOptions = [
    ...new Set([...realEstateAgents, ...propertyOwners]),
  ];

  // Get the selected option value from propertyData
  const propertyOwnerSelectedOption = propertyOwnerOptions.find(
    (option) => option.value === propertyData?.user_id
  );
  const propertyTypeSelectedOption = propertyTypeOptions.find(
    (option) => option.value === propertyData?.type
  );
  const propertyOfferSelectedOption = propertyOfferOptions.find(
    (option) => option.value === propertyData?.offer
  );

  const townSelectedOption = townList.find(
    (option) => option.value === propertyData?.town
  );
  const quarterSelectedOption = quarterList.find(
    (option) => option.value === propertyData?.quarter
  );
  return (
      ...
      <Row>
        ...
        <Col lg={12}>
          ...
          <Form onSubmit={handleSubmit(onSubmit)}>
            ...
            <section
              id="basic-info"
              className="p-4 mb-4 border-0 shadow-sm card card-body"
            >
                ...
                <Row>
                  ...
                  >
                    ...
                    <Select
                      {...register("user_id")}
                      name="user_id"
                      value={propertyManagerOptions.find(
                        (option) => option.value === propertyData?.user_id
                      )} // Pre-select based on propertyData
                      onChange={(selected) =>
                        handleChange({
                          target: {
                            name: "user_id",
              ...
              <Alert variant="info" className="mb-4 d-flex">
                ...
                <p className="mb-1 fs-sm">
                  La taille maximale des images est 8M de formats: jpeg, jpg,
                  png. L'image principale d'abord.
                  <br />
                  La taille max des videos est 10M de formats: mp4, mov.
                </p>
              </Alert>
              <FilePond
                ...
                labelMaxFileSizeExceeded={
                  "La taille maximale des fichiers est 10M"
                }
            ...
            >
              ...
              <Row>
                ...
                >
                  <Form.Label className="pb-1 mb-2 d-block fw-bold">
                    Préciser l'adresse de référence{" "}
                    <span className="text-danger">*</span>
                  </Form.Label>
              ...
              <Form.Label className="pt-3 pb-2 fw-bold">
                Localiser le bien immobilier sur la carte ci-dessous
              </Form.Label>
              <MapContainer
                center={userLocation}
                zoom={13}
                ...
                <CustomMarker position={[6.1611048, 1.1909014]} icon="dot">
                  <Popup>
                    <div className="p-3">
                      ...
                      <p className="pt-1 mb-0 fs-xs text-muted mt-n3">
                        Non loin de Oasis Zener Agoe 2 Lions
                      </p>
                    </div>
                  </Popup>
                </CustomMarker>
            ...
            >
              ...
              <Row>
                <Form.Group controlId="area" className="mb-4" as={Col} sm={4}>
                  ...
                  <Form.Control
                    ...
                    className={`form-control ${
                      errors.area ? "is-invalid" : "is-valid"
                    }`}
                  />
                  {errors.area && (
                    <Form.Control.Feedback type="invalid" tooltip>
                ...
                >
                  ...
                  <Form.Control
                    ...
                    onChange={handleChange}
                    className={`form-control ${
                      errors.bedRooms ? "is-invalid" : "is-valid"
                    }`}
                  />
                  {errors.bedRooms && (
                    <Form.Control.Feedback type="invalid" tooltip>
                ...
                >
                  <Form.Label className="pb-1 mb-2 d-block fw-bold">
                    Menages <span className="text-danger">*</span>
                  </Form.Label>
                  <Form.Control
                    {...register("household")}
                    type="number"
                ...
                >
                  <Form.Label className="pb-1 mb-2 d-block fw-bold">
                    Terrasses<span className="text-danger">*</span>
                  </Form.Label>
                  <Form.Control
                    {...register("terraces")}
                    type="number"
                ...
                >
                  <Form.Label className="pb-1 mb-2 d-block fw-bold">
                    Balcons <span className="text-danger">*</span>
                  </Form.Label>
                  <Form.Control
                    {...register("balcony")}
                    type="number"
                ...
                <Form.Group as={Col} md={4} controlId="water" className="mb-3">
                  <Form.Label className="pb-1 mb-2 d-block fw-bold">
                    Eau <span className="text-danger">*</span>
                  </Form.Label>
                  <Form.Select
                    {...register("water")}
                    defaultValue="0"
                ...
                >
                  <Form.Label className="pb-1 mb-2 d-block fw-bold">
                    Electricité <span className="text-danger">*</span>
                  </Form.Label>
                  <Form.Select
                ...
                <Form.Group as={Col} sm={4} controlId="garden" className="mb-3">
                  <Form.Label className="pb-1 mb-2 d-block fw-bold">
                    Jardin<span className="text-danger">*</span>
                  </Form.Label>
                  <Form.Check
                    type="radio"
                    name="garden"
                ...
                <Form.Group as={Col} sm={4} controlId="pool" className="mb-3">
                  <Form.Label className="pb-1 mb-2 d-block fw-bold">
                    Piscine<span className="text-danger">*</span>
                  </Form.Label>
                  <Form.Check
                    type="radio"
                    name="pool"
              ...
              <div className="pt-3 pb-2 form-label fw-bold">
                Le bien immobilier est meublé?
              </div>
              <Form.Check
                type="radio"
                name="furnished"
            ...
            >
              ...
              {propertyData.offer == "1" && (
                <>
                  <Form.Label
                    htmlFor="monthPrice"
                    className="pb-1 mb-2 d-block fw-bold"
                  >
            ...
            >
              ...
              <Row>
                <Form.Group
                  as={Col}
                  sm={4}
                  controlId="cautionGuarantee"
                  className="mb-3"
                >
                  <Form.Label className="pb-1 mb-2 d-block fw-bold">
                    Caution & Avances <span className="text-danger">*</span>
                  </Form.Label>
                  <Form.Control
                    type="number"
                    defaultValue="6"
                    placeholder="Nombre de cautions plus Avances"
                    required
                ...
                >
                  <Form.Label className="pb-1 mb-2 d-block fw-bold">
                    Honoraires <span className="text-danger">*</span>
                  </Form.Label>
                  <Form.Control
                    type="number"
                    defaultValue="1"
                ...
                >
                  <Form.Label className="pb-1 mb-2 d-block fw-bold">
                    Gardien de sécurité <span className="text-danger">*</span>
                  </Form.Label>
                  <Form.Check
                    type="radio"
                    name="security"
                ...
                >
                  <Form.Label className="pb-1 mb-2 d-block fw-bold">
                    Présence du bailleur <span className="text-danger">*</span>
                  </Form.Label>
                  <Form.Check
                    type="radio"
                    name="owner"
                ...
                >
                  ...
                  <Form.Control
                    ...
                    rows={5}
                    defaultValue=""
                    placeholder="Par exemple: Pas de célibataires"
                  />
                </Form.Group>
              </Row>
            </section>
