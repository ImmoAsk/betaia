...
const PropertyInventoryPage = ({ _newInventory, _acceptedInventory, _declinedInventory }) => {
  ...
  const createPropertyInventoryModal = (e) => {
    if (session) {
  ...
  const getHandledPropertyInventorys = (projects) => {
    return <PropertyInventoryList projects={projects} />;
  };
  ...
  return (
    <RealEstatePageLayout pageTitle='Etat de lieux' activeNav='Account' userLoggedIn>
      {PropertyInventoryShow && (
        <PropertyInventoryModal centered size='lg' show={PropertyInventoryShow} onHide={() => setPropertyInventoryShow(false)}/>
      )}
      <RealEstateAccountLayout accountPageTitle='Etat de lieux'>
        <div className='d-flex align-items-center justify-content-between mb-3'>
          <h1 className='h2 mb-0'>Etat de lieux</h1>
          <div className='d-flex align-items-right'>
            <a href='#' className='fw-bold text-decoration-none' onClick={createPropertyInventoryModal}>
        ...
        <p className='pt-1 mb-4'>
          Trouvez ici tous les etats des lieux pour les biens immobiliers en location et bails
            ...
            <>
              {activeTab === 'published' && (
                <Col xs={12} style={columnStyle}>
                  {getHandledPropertyInventorys(_newInventory)}
                </Col>
              )}
              {activeTab === 'accepted' && (
                <Col xs={12} style={columnStyle}>
                  {getHandledPropertyInventorys(_acceptedInventory)}
                </Col>
              )}
              {activeTab === 'declined' && (
                <Col xs={12} style={columnStyle}>
                  {getHandledPropertyInventorys(_declinedInventory)}
                </Col>
              )}
            </>
          ) : (
...
export async function getServerSideProps(context) {
  ...
  switch (session?.user?.roleId) {
    case '151': // Tenant role
      ...
      _acceptedInventory = await fetchRenterInventoryByStatut(1, session.user.id);
      _declinedInventory = await fetchRenterInventoryByStatut(2, session.user.id);
      break;
