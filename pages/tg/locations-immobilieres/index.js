...
export async function getServerSideProps(context) {  
  ...
  let dataAPIresponse = await fetch(`https://immoaskbetaapi.omnisoft.africa/public/api/v2?query={getPropertiesByKeyWords(limit:800,orderBy:{column:NUO,order:DESC},offre_id:"${offreId}")
  {badge_propriete{badge{badge_name,badge_image}},visuels{uri,position},id,surface,nuitee,lat_long,nuo,usage,offre{denomination},categorie_propriete{denomination},pays{code},piece,titre,garage,cout_mensuel,ville{denomination},wc_douche_interne,cout_vente,quartier{denomination}}}`);
