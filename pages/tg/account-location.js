...
const AccountLocationPage = () => {
  ...
  useQuery({
    queryKey: ["RTProperties", user_id],
    enabled: !!user_id,
    queryFn: () => {
      const query = `{getUserProperties(user_id:${user_id},first:5){data{surface,badge_propriete{badge{badge_name,badge_image}},id,nuo,usage,offre{denomination},categorie_propriete{denomination},pays{code},piece,titre,garage,cout_mensuel,ville{denomination},wc_douche_interne,cout_vente,quartier{denomination},visuels{uri}}}}`;
      return axios.get('/api/graphql', { params: { query } });
    },
    onSuccess: (res) => {
      const data = res.data?.data?.getUserProperties?.data || [];
      setProperties(data.map((property) => {
        const paysCode = property?.pays?.code || 'tg';
        return {
          href: getPropertyFullUrl(paysCode, property.offre.denomination, property.categorie_propriete.denomination, property.ville.denomination, property.quartier?.denomination, property.nuo),
          images: getFirstImageArray(property.visuels),
          title: 'N' + property.nuo + ': ' + property.categorie_propriete.denomination + ' a ' + property.offre.denomination + ' | ' + property.surface + 'm2',
          category: property.usage,
          location: (property.quartier?.denomination || 'Centre') + ", " + property.ville.denomination,
