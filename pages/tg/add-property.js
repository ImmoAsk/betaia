...
const AddPropertyPage = () => {
  ...
  const [isFurnished, setIsFurnished] = useState(0);
  const [propertyTitle, setPropertyTitle] = useState("");
  const [propertyOwner, setPropertyOwner] = useState(session?.user?.id || 0);
  const [propertyType, setPropertyType] = useState(0);
  const [propertyHouseHold, setPropertyHouseHold] = useState('0');

  // 1 is real estate agent, 2 is real estate owner
  const [propertyUserRole, setPropertyUserRole] = useState(1);
  const [propertyOffer, setPropertyOffer] = useState(0);
  const [propertySecurity, setPropertySecurity] = useState(0);
  const [propertyCountry, setPropertyCountry] = useState(228);
  ...
  const validationSchema = Yup.object().shape({
    propertyTown: Yup.string().required('Preciser la ville svp'),
    propertyOffer: Yup.string().required('Preciser l\'offre immobilière svp'),
    propertyAddress: Yup.string().required('Indiquer une adresse commune de référence'),
    propertyType: Yup.string().required('Selectionner le type du bien immobilier'),
    propertyQuarter: Yup.string().required('Selectionner le quartier'),
  });
  ...
  // Function to build and append map file upload
  const buildAppendMapFileUpload = (event) => {
    event.stopPropagation();
    event.preventDefault();
    ...
    const _formData = JSON.stringify({
      ...
      part_min_investissement: Number(propertyInvestmentPrice),
      usage: 1,
      garage: Number(parkingsValue), // Assuming parkingsValue is defined elsewhere
      nuo: 4040,
      eau: Number(propertyWater),
      electricite: Number(propertyElectricity),
      categorie_id: Number(propertyType),
      ...
      ville_id: Number(propertyTown),
      quartier_id: Number(propertyQuarter),
      adresse_id: propertyAddress,
      lat_long: "6.12564358,1.1568922", // Consider making this dynamic if necessary
      piscine: Number(propertyPool),
      gardien_securite: Number(propertySecurity),
      salon: Number(propertyLivingRooms),
      cuisine: Number(propertyKitchen),
      ...
      terrasse_balcon: Number(propertyTerraces),
      cout_visite: Number(propertyVisitRight),
      wc_douche_interne: Number(InsideBathRoomsValue), // Assuming InsideBathRoomsValue is defined elsewhere
      wc_douche_externe: Number(OutsideBathRoomsValue), // Assuming OutsideBathRoomsValue is defined elsewhere
      url: null,
    });
    ...
    axios.post('https://immoaskbetaapi.omnisoft.africa/public/api/v2', formData)
      .then((response) => {
        console.log(JSON.stringify(JSON.parse(response.data)));
      })
      .catch((error) => {
