  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      if (!newPropertyData.titre || !newPropertyData.offre_id) {
        // Check 'titre' and 'offer' as per PropertyForm
        return res
          .status(400)
          .json({ error: "Titre et type d'offre sont requis." });
      }
      ...
      Object.keys(files).forEach((fieldName) => {
        // Corrected field name check from image_ to images[
        if (fieldName.startsWith("images[")) {
          ...
          fileArray.forEach((file) => {
            if (file && file.newFilename) {
              ...
              const webPath = `/uploads/properties/${file.newFilename}`;
      ...
      const newProperty = {
        ...
        images:
          uploadedImagePaths.length > 0
            ? uploadedImagePaths
            : [
                `https://via.placeholder.com/300x200.png?text=Property+${Math.floor(
                  Math.random() * 1000
                )}`,
              ], // Use uploaded images
        ...
        offre_id:
          Number(newPropertyData.offre_id || newPropertyData.offer) || 0,
