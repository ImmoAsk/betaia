...
async function writeUserSocialTokens(tokens) {
  // ensureTokensFileExists() is called by readUserSocialTokens, so not strictly needed here if read is always called first.
  // However, for safety, or if writeUserSocialTokens could be called independently:
...
export default async function handler(req, res) {
  ...
  res.setHeader('Set-Cookie', 'facebook_oauth_state=; Path=/; HttpOnly; Max-Age=0; SameSite=Lax');

  if (state !== expectedState) { // Check if state exists and matches
    console.error('Invalid OAuth state for Facebook:', { received: state, expectedFromCookie: expectedState });
    return res.redirect('/social-media?status=facebook_error&error_description=Invalid_state');
  }

  if (error) {
    console.error('Facebook OAuth Error:', error, error_description);
    return res.redirect(`/social-media?status=facebook_error&error_description=${encodeURIComponent(error_description || error)}`);
  }

  if (!code) {
    console.error('Facebook OAuth: No code received.');
    return res.redirect('/social-media?status=facebook_error&error_description=Authorization_code_missing');
  }

  const { FACEBOOK_CLIENT_ID, FACEBOOK_CLIENT_SECRET, NEXT_PUBLIC_FACEBOOK_REDIRECT_URI, TOKEN_ENCRYPTION_KEY } = process.env;

  if (!FACEBOOK_CLIENT_ID || !FACEBOOK_CLIENT_SECRET || !NEXT_PUBLIC_FACEBOOK_REDIRECT_URI) {
    console.error('Facebook environment variables not set.');
    return res.redirect('/social-media?status=facebook_error&error_description=Configuration_error_FB_credentials');
  }
  if (!TOKEN_ENCRYPTION_KEY || TOKEN_ENCRYPTION_KEY.length !== 32) {
    console.error('TOKEN_ENCRYPTION_KEY is missing or not 32 bytes long. Please check your .env.local file.');
    return res.redirect('/social-media?status=facebook_error&error_description=Server_encryption_key_error');
  }

  try {
    // 1. Exchange authorization code for access token
    const tokenResponse = await fetch(`https://graph.facebook.com/v12.0/oauth/access_token?client_id=${FACEBOOK_CLIENT_ID}&redirect_uri=${NEXT_PUBLIC_FACEBOOK_REDIRECT_URI}&client_secret=${FACEBOOK_CLIENT_SECRET}&code=${code}`);
    const tokenData = await tokenResponse.json();

    if (tokenData.error) {
      console.error('Facebook Token Exchange Error:', tokenData.error);
      return res.redirect(`/social-media?status=facebook_error&error_description=${encodeURIComponent(tokenData.error.message || 'Token_exchange_failed')}`);
    }
    ...
    if (profileData.error) {
      ...
      return res.redirect(`/social-media?status=facebook_error&error_description=${encodeURIComponent(profileData.error.message || 'Profile_fetch_failed')}`);
    }
    ...
    try {
      ...
      userSocialTokens = userSocialTokens.filter(token => !(token.userId === MOCK_USER_ID && token.platform === 'facebook'));
      ...
      console.error('Facebook Callback: Error during token file operation or encryption:', fileOrEncryptError.message, fileOrEncryptError.stack);
      throw new Error('Failed to process and store Facebook tokens due to a server-side issue.');
    }
    ...
    console.error('Facebook OAuth Callback Handler Error:', err.message, err.stack);
    let errorMessage = 'An_unexpected_error_occurred';
    if (err instanceof Error) {
      errorMessage = err.message;
    }
    return res.redirect(`/tg/account-socialmedia?status=facebook_error&error_description=${encodeURIComponent(errorMessage)}`);
  }
}
